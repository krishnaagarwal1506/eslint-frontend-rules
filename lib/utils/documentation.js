// `export function Foo() {}` / `export const Foo = () => {}` wrap the
// declaration in an ExportNamedDeclaration/ExportDefaultDeclaration node —
// the declaration's own `.parent` is that wrapper, not Program, and any
// leading doc comment sits before the `export` keyword (attached to the
// wrapper), not before the inner declaration. Both `isRootLevel` and the
// doc-comment/insertion-point logic need to look at the wrapper when one
// exists, or two bugs follow: the rule never matches an exported
// declaration at all (previously true for essentially every real
// component/hook, since unexported ones are rare), and even where it does
// fire, an existing JSDoc above `export` goes undetected.
function getDocTarget(node) {
  if (
    node.parent &&
    (node.parent.type === "ExportNamedDeclaration" ||
      node.parent.type === "ExportDefaultDeclaration")
  ) {
    return node.parent;
  }
  return node;
}

function hasJSDoc(node, sourceCode) {
  const jsdoc = sourceCode
    .getCommentsBefore(getDocTarget(node))
    .find(
      (comment) => comment.type === "Block" && comment.value.startsWith("*"),
    );
  return Boolean(jsdoc);
}

function isRootLevel(node) {
  if (!node.parent) return false;
  if (node.parent.type === "Program") return true;
  const target = getDocTarget(node);
  return (
    target !== node && !!target.parent && target.parent.type === "Program"
  );
}

/**
 * Factory to create a JSDoc rule for a specific function type.
 * @param {Object} options
 * @param {(name: string) => boolean} typePredicate - Returns true if the function matches the type (component/hook/root).
 * @param {string} messageId - The messageId for reporting.
 * @param {string} defaultMessage - The default error message.
 * @returns {import('eslint').Rule.RuleModule}
 */
function createJsdocRule({ typePredicate, messageId, defaultMessage }) {
  const micromatch = require("micromatch");
  return {
    meta: {
      type: "suggestion",
      docs: {
        description: defaultMessage,
        category: "Documentation",
        recommended: false,
      },
      fixable: "code",
      schema: [
        {
          type: "object",
          properties: {
            folders: {
              type: "array",
              items: { type: "string" },
              description:
                "Only apply rule to files in these folders (micromatch patterns)",
            },
          },
          additionalProperties: false,
        },
      ],
      messages: {
        [messageId]: defaultMessage,
      },
    },
    create(context) {
      const options = context.options[0] || {};
      const folders = options.folders || null;
      const filename = context.filename;
      if (
        folders &&
        !folders.some((pattern) => micromatch.isMatch(filename, pattern))
      ) {
        return {};
      }
      const sourceCode = context.sourceCode;
      function check(node) {
        if (!isRootLevel(node)) return;
        const name = node.id ? node.id.name : null;
        if (!typePredicate(name)) return;
        if (hasJSDoc(node, sourceCode)) return;
        context.report({
          node,
          messageId,
          data: { name: name || "(anonymous)" },
          fix(fixer) {
            const jsdoc = `/** ${defaultMessage.replace("{{name}}", name || "(anonymous)").replace(/\. *$/, "")} */\n`;
            return fixer.insertTextBefore(getDocTarget(node), jsdoc);
          },
        });
      }
      return {
        FunctionDeclaration: check,
        VariableDeclaration(node) {
          if (!isRootLevel(node)) return;
          for (const decl of node.declarations) {
            const name = decl.id && decl.id.name;
            if (
              decl.init &&
              (decl.init.type === "ArrowFunctionExpression" ||
                decl.init.type === "FunctionExpression") &&
              typePredicate(name) &&
              !hasJSDoc(node, sourceCode)
            ) {
              context.report({
                node: decl,
                messageId,
                data: { name: name || "(anonymous)" },
                fix(fixer) {
                  const jsdoc = `/** ${defaultMessage.replace("{{name}}", name || "(anonymous)").replace(/\. *$/, "")} */\n`;
                  return fixer.insertTextBefore(getDocTarget(node), jsdoc);
                },
              });
            }
          }
        },
      };
    },
  };
}

module.exports = { hasJSDoc, isRootLevel, createJsdocRule };
